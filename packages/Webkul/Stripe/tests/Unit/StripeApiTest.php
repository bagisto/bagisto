...
/**
 * Set a Stripe payment method setting for the default channel.
 */
function setStripeSetting(string $field, string $value): void
{
...
beforeEach(function () {
    setStripeSetting('sandbox', '1');

    setStripeSetting('api_test_key', 'sk_test_fake_key');

    setStripeSetting('webhook_secret', 'whsec_live_secret');

    setStripeSetting('webhook_test_secret', 'whsec_test_secret');
...
it('should read the webhook signing secret of the mode the store runs in', function (string $sandbox, string $secret) {
    setStripeSetting('sandbox', $sandbox);
...
it('should convert an amount from the smallest unit of its currency', function () {
    Currency::updateOrCreate(['code' => 'JPY'], ['name' => 'Japanese Yen', 'symbol' => '¥', 'decimal' => 0]);

    expect($this->stripe->fromStripeAmount(13250, core()->getBaseCurrencyCode()))->toBe(13250 / (10 ** (core()->getBaseCurrency()->decimal ?? 2)))
        ->and($this->stripe->fromStripeAmount(1500, 'jpy'))->toBe(1500.0)
