...
beforeEach(function () {
    $this->setConfig([
        ...
        'sales.payment_methods.stripe.webhook_test_secret' => 'whsec_test_bagisto',
    ]);
...
it('should refuse every event while no signing secret is set', function () {
    $this->setConfig('sales.payment_methods.stripe.webhook_test_secret', '');
...
it('should place the order for a paid checkout the customer never came back from', function () {
    ...
    expect($order)->not->toBeNull()
        ...
        ->and(OrderTransaction::query()->where('transaction_id', 'pi_test_'.$cart->id)->value('order_id'))->toBe($order->id)
        ->and($cart->fresh()->is_active)->toBeFalse();
});
...
it('should place one order however often Stripe and the customer report the payment', function () {
    ...
    $this->get(route('stripe.payment.success', ['session_id' => 'cs_test_'.$cart->id]))
        ...
        ->assertSessionHas('order_id', Order::query()->where('cart_id', $cart->id)->value('id'));
...
it('should place the order for a delayed payment once Stripe reports it succeeded', function () {
    ...
    expect(Order::query()->where('cart_id', $cart->id)->value('status'))->toBe(Order::STATUS_PROCESSING);
});
...
it('should record a partial refund made in Stripe once, however often Stripe reports it', function () {
    ...
    expect(Refund::query()->where('order_id', $order->id)->count())->toBe(1)
        ->and($order->base_grand_total_refunded)->toBePrice($half)
        ->and($order->status)->toBe(Order::STATUS_PROCESSING)
        ->and(OrderComment::query()->where('order_id', $order->id)->count())->toBe(1);
});
...
it('should refund the items and close the order once Stripe refunds the whole payment', function () {
    ...
    expect($order->base_grand_total_refunded)->toBePrice((float) $order->base_grand_total)
        ...
        ->and($refund->base_grand_total)->toBePrice((float) $order->base_grand_total);
});
...
it('should refund the remaining items once Stripe refunds the rest of a partly refunded payment', function () {
    ...
    $lastRefund = Refund::query()->where('order_id', $order->id)->latest('id')->first();

    expect(Refund::query()->where('order_id', $order->id)->count())->toBe(2)
        ->and($order->base_grand_total_refunded)->toBePrice((float) $order->base_grand_total)
        ->and($order->status)->toBe(Order::STATUS_CLOSED)
        ->and($lastRefund->items)->not->toBeEmpty()
        ->and($lastRefund->base_grand_total)->toBePrice($order->base_grand_total - $half);
});
...
it('should place the order first when a refund arrives before it', function () {
    ...
    expect($order)->not->toBeNull()
        ->and($order->base_grand_total_refunded)->toBePrice(1)
...
it('should note on the order when a dispute opens and when it closes, once each', function () {
    ...
    $comments = OrderComment::query()->where('order_id', $order->id)->orderBy('id')->pluck('comment');
...
it('should place the order first when a dispute arrives before it', function () {
    ...
    expect($order)->not->toBeNull()
        ->and(OrderComment::query()->where('order_id', $order->id)->value('comment'))->toContain('fraudulent');
});
