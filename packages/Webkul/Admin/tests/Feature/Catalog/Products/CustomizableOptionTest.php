 ...
 */
function makeProductWithSelectOption(): array
{
...
/**
 * Build the form input the product edit page sends for one customizable option.
 */
...
it('should give an option switched to a single price type one new price in place of its values', function (string $priceKey) {
    [$product, $option] = makeProductWithSelectOption();
    ...
    expect($option->type)->toBe('text')
        ...
        ->and((float) $option->customizable_option_prices->first()->price)->toBe(4.0);
})->with([
...
it('should keep the options sent, add the new ones and remove the rest', function () {
    [$product, $kept] = makeProductWithSelectOption();
...
it('should never update an option or value that belongs to another product', function () {
    [$product] = makeProductWithSelectOption();

    [, $otherOption] = makeProductWithSelectOption();
