...
export class RmaStorefrontPage extends BasePage {
    ...
    private async contrastRatio(locator: Locator): Promise<number> {
        const colours = await locator.evaluate((element) => {
            ...
            let background = readable(style.backgroundColor);

            while (node && background.alpha === 0) {
                node = node.parentElement;

                background = node
                    ? readable(window.getComputedStyle(node).backgroundColor)
                    : { rgb: [255, 255, 255], alpha: 1 };
            }

            return {
                text: readable(style.color).rgb,
                background: background.rgb.length
                    ? background.rgb
                    : [255, 255, 255],
            };
        });
        ...
        const text = luminance(colours.text);
        const background = luminance(colours.background);
