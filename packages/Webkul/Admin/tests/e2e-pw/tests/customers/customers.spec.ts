...
async function createCustomer(adminPage) {
    ...
    await expect(adminPage.getByText('Customer created successfully')).toBeVisible();
}
...
test.describe('customer management', () => {
    ...
    test('edit customer', async ({ adminPage }) => {
        ...
        await adminPage.waitForSelector('div[class="flex cursor-pointer items-center justify-between gap-1.5 px-2.5 text-blue-600 transition-all hover:underline"]:visible', { state: 'visible' });
        const createBtn = await adminPage.$$('div[class="flex cursor-pointer items-center justify-between gap-1.5 px-2.5 text-blue-600 transition-all hover:underline"]:visible');
        ...
        await expect(adminPage.getByText('Customer Updated Successfully')).toBeVisible();
    });
    ...
    test('Add address', async ({ adminPage }) => {
        ...
        await expect(adminPage.getByText('Address Created Successfully')).toBeVisible();
    });
    ...
    test('edit address', async ({ adminPage }) => {
        ...
        const createBtn = await adminPage.$$('p[class="cursor-pointer text-blue-600 transition-all hover:underline"]:visible');
        ...
        await expect(adminPage.getByText('Address Updated Successfully')).toBeVisible();
    });
    ...
    test('delete address', async ({ adminPage }) => {
        ...
        await adminPage.waitForSelector('p[class="cursor-pointer text-red-600 transition-all hover:underline"]:visible');
    ...
    test('mass update customers', async ({ adminPage }) => {
        ...
        await expect(adminPage.getByText('Selected Customers successfully updated')).toBeVisible();
    });
});
