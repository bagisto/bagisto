...
test.describe('attribute management', () => {
    test('should create a new attribute successfully', async ({ adminPage }) => {
        ...
        // Label Section
        await adminPage.fill('input[name="admin_name"]', concatenatedNames);
        await adminPage.fill('input[name="en[name]"]', concatenatedNames);

        // General Section
        await adminPage.fill('#code', concatenatedNames);
        ...
        await adminPage.click('label[for="is_configurable"]');
        ...
        await expect(adminPage.locator('input#value_per_channel')).toBeChecked();
        await expect(adminPage.locator('input#is_configurable')).toBeChecked();
        await expect(adminPage.locator('input#is_visible_on_front')).toBeChecked();
        await expect(adminPage.locator('input#is_comparable')).toBeChecked();

        // Submit
        await adminPage.click('button[type="submit"]');

        await expect(adminPage.getByText('Attribute Created Successfully')).toBeVisible();
    });
    ...
    test('should validate required fields', async ({ adminPage }) => {
        ...
        await expect(adminPage.locator('text=The Admin field is required')).toBeVisible();
        await expect(adminPage.locator('text=The Attribute Code field is required')).toBeVisible();
    });
    ...
    test('should mass delete and existing attributes', async ({ adminPage }) => {
        ...
        let selectActionButton = await adminPage.waitForSelector('button:has-text("Select Action")');
        ...
        await expect(adminPage.getByText(/Attribute Deleted Successfully|Attribute Deleted Failed/)).toBeVisible();
    });
});
