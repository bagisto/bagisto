...
async function createPage(adminPage) {
    ...
    await adminPage.waitForSelector('a.primary-button:has-text("Create Page")', { state: "visible" });
...
test.describe("cms management", () => {
    ...
    test("should edit a page", async ({ adminPage }) => {
        ...
        await adminPage.getByRole('textbox', { name: 'Page Title' }).fill(cms.name);

        /**
         * Edit Description Section.
         */
        const editorFrame = adminPage.frameLocator('iframe.tox-edit-area__iframe');
    ...
    test("should delete multiple selected pages", async ({ adminPage }) => {
        /**
