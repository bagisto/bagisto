<div class="flex gap-[6px] px-[16px] py-[8px] border-b border-gray-300">
    {{-- Date Filter --}}
    <div class="mb-[5px] flex-1">
        <div class="shimmer w-[160px] h-[24px] mb-[2px]"></div>

        {{-- Start Date --}}
        <div class="shimmer w-[160px] h-[40px] mb-[2px]"></div>
    </div>

    <div class="mb-[5px] gap-[4px] flex-1">
        <div class="shimmer w-[160px] h-[24px] mb-[2px]"></div>

        {{-- End Date --}}
        <div class="shimmer w-[160px] h-[40px] mb-[2px]"></div>
    </div>
</div>

<div class="h-[301px] w-full grid gap-[16px] px-[16px] py-[8px] border-b border-gray-300">
    {{-- Total Sales --}}
    <div class="w-full h-[38px] flex gap-[8px] justify-between">
        <div class="flex flex-col gap-[4px] justify-between">
            <div class="shimmer w-[85px] h-[17px]"></div>

            {{-- Total Sales Amount --}}
            <div class="shimmer w-[85px] h-[17px]"></div>
        </div>

        <div class="flex flex-col gap-[4px] justify-between">
            {{-- Date --}}
            <div class="shimmer w-[83px] h-[17px]"></div>

            {{-- Total Orders --}}
            <div class="shimmer w-[56px] h-[17px] self-end"></div>
        </div>
    </div>

    {{-- Graph Chart --}}
    <div class="shimmer w-full h-[228px]"></div>
</div>