...
@push('scripts')
    ...
    <script type="text/x-template" id="create-coupon-form-template">
        <div class="">
            ...
            <datagrid-plus src="{{ route('admin.cart_rules_coupons.index', $cartRule->id) }}"></datagrid-plus>
        </div>
    </script>
            ...
            methods: {
                generateCoupons: function(formScope) {
                    ...
                    this.$validator.validateAll(formScope).then(function (result) {
                        if (result) {
                            self.$http.post("{{ route('admin.cart_rules_coupons.store', $cartRule->id) }}", self.coupon_format)
