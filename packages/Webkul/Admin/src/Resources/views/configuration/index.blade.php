    ...
    <div class="grid gap-y-[30px]">
        @foreach ($config->items as $itemKey => $item)
            <div>
                ...
                <div class="grid grid-cols-4 gap-12 flex-wrap justify-between p-[16px] mt-[8px] bg-white dark:bg-gray-900 rounded-[4px] box-shadow max-1580:grid-cols-3 max-xl:grid-cols-2 max-sm:grid-cols-1">
                    <!-- Menus cards -->
                    @foreach ($item['children'] as $childKey =>  $child)
                        <a 
                            class="flex items-center gap-2 max-w-[360px] p-[8px] rounded-[8px] transition-all hover:bg-gray-100 dark:hover:bg-gray-950"
