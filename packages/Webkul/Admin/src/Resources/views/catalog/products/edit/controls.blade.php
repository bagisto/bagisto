@switch($attribute->type)
    ...
    @case('price')
        <div class="relative">
            <span class="absolute ltr:left-4 rtl:right-4 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-300 {{ $attribute->code == 'price' ? 'text-xl leading-none' : '' }}">
            ...
            <x-admin::form.control-group.control
                ...
                :label="$attribute->admin_name"
                value="{{ old($attribute->code) ?: $product[$attribute->code] }}"
                :class="'ltr:pl-8 rtl:pr-8 ' . ($attribute->code == 'price' ? 'py-2 bg-gray-50 text-xl leading-none font-bold' : '')"
            >
            </x-admin::form.control-group.control>
        </div>
        ...
        <div class="flex gap-2.5">
                ...
                >
                    ...
                    @else
                        <div class="inline-flex w-full max-w-max cursor-pointer appearance-none items-center justify-between gap-x-1 rounded-md border border-transparent p-1.5 text-center text-gray-600 dark:text-gray-300 transition-all marker:shadow hover:bg-gray-200 dark:hover:bg-gray-800 active:border-gray-300">
                            <i class="icon-down-stat text-2xl leading-none"></i>
                        </div>
                    @endif
                </a>
            ...
            >
                <input
                    ...
                    id="{{ $attribute->code }}"
                    :class="[errors['{{ $attribute->code }}'] ? 'border border-red-600 hover:border-red-600' : '']"
                    class="flex w-full min-h-[39px] py-2 px-3 border rounded-md text-sm leading-none text-gray-600 dark:text-gray-300 transition-all hover:border-gray-400 dark:hover:border-gray-400 dark:file:bg-gray-800 dark:file:dark:text-white focus:border-gray-400 dark:focus:border-gray-400 dark:border-gray-800"
                    @change="handleChange"
                    @blur="handleBlur"
                >
            </v-field>
