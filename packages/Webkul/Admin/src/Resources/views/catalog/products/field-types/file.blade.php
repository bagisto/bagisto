@if ($product[$attribute->code])
    <a href="{{ route('admin.catalog.products.file.download', [$product->id, $attribute->id] )}}" target="_blank">
        <i class="icon sort-down-icon download"></i>
    </a>
@endif

<input type="hidden" name="File[path]" value="{{ old($attribute->code) ?: $product[$attribute->code] }}">
...
@if ($product[$attribute->code])
    <div class="control-group" style="margin-top: 5px;">
        <span class="checkbox">
            <input type="checkbox" id="{{ $attribute->code }}[delete]"  name="file_delete" value="1">
