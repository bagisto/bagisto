<?php

return [
    'response' => [
        'provide-credentials' => 'בבקשה ספקו בעדויות PhonePe חוקיות.',
        'cart-not-found' => 'לא נמצא עגלה פעילה. אנא הוסיפו פריטים לעגלה שלכם לפני המעבר לתשלום.',
        'phonepe-payment-reference-missing' => 'התייחסות לתשלום PhonePe חסרה.',
        'phonepe-payment-pending' => 'התשלום שלך ב-PhonePe עדיין ממתין. אנא השלימו אותו או המתינו רגע ובדקו שוב.',
        'phonepe-payment-failed' => 'התשלום שלך ב-PhonePe נכשל. אנא נסו שוב.',
        'invalid-transaction' => 'עסקה לא חוקית. אנא נסו שוב.',
    ],
];
