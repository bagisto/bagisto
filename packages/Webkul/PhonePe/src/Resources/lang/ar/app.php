<?php

return [
    'response' => [
        'provide-credentials' => 'يرجى تقديم بيانات اعتماد PhonePe صحيحة.',
        'cart-not-found' => 'لم يتم العثور على سلة نشطة. يرجى إضافة العناصر إلى سلتك قبل الانتقال إلى الدفع.',
        'phonepe-payment-reference-missing' => 'مرجع الدفع PhonePe مفقود.',
        'phonepe-payment-pending' => 'دفع PhonePe الخاص بك قيد الانتظار. يرجى إكماله أو انتظر قليلاً وتحقق مرة أخرى.',
        'phonepe-payment-failed' => 'فشل الدفع عبر PhonePe. يرجى محاولة مرة أخرى.',
        'invalid-transaction' => 'معاملة غير صالحة. يرجى محاولة مرة أخرى.',
    ],
];
