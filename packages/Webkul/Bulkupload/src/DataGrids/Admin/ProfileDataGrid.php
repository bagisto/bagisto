...
class ProfileDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        $queryBuilder = DB::table('bulkupload_data_flow_profiles')
            ...
            ->leftJoin('customers', 'bulkupload_data_flow_profiles.seller_id', '=', 'customers.id')
            ->select('bulkupload_data_flow_profiles.id',
            DB::raw('CONCAT(customers.first_name, " ", customers.last_name) as seller_id'),
            'bulkupload_data_flow_profiles.profile_name', 'attribute_families.name', 'bulkupload_data_flow_profiles.created_at');
