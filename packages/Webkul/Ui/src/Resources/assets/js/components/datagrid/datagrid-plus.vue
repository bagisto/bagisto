          ...
          >
            <div class="control-group">
              <select
                ...
                name="customer_group"
                @change="changeExtraFilter($event, 'customer_group')"
              >
                <option
                  value="all"
                ...
                <option
                  ...
                  ) in extraFilters.customer_groups"
      ...
      <table class="table">
            ...
            <th colspan="100%">
              ...
              >
                ...
                <form
                  ...
                  action=""
                  :onsubmit="`return confirm('${massActionConfirmText}')`"
                >
                  ...
                  <div class="control-group">
                    ...
                    >
                      <option
                        v-for="(massAction, index) in massActions"
                  ...
                  >
                    ...
                    >
                      <option
                        :key="id"
                        v-for="(massActionValue, id) in massActionValues"
        ...
        <tbody>
          <template v-if="records.data.length">
            <tr :key="recordKey" v-for="(record, recordKey) in records.data">
              ...
              >
                <div class="action">
                  <a
                    ...
                    v-if="record[`${action.title.toLowerCase()}_to_display`]"
                    :id="
                      record[
                        typeof action.index !== 'undefined' && action.index
                          ? action.index
                          : index
                      ]
                    "
                    :href="
                      action.method == 'GET'
                        ? record[`${action.title.toLowerCase()}_url`]
                        : 'javascript:void(0);'
                    "
                    v-on:click="
                      action.method != 'GET'
                        ? typeof action.function !== 'undefined' &&
                          action.function
                          ? action.function
                          : doAction($event)
                        : {}
                    ...
                    :data-token="csrf"
                    :target="
                      typeof action.target !== 'undefined' && action.target
                        ? action.target
                        : ''
                    "
                    :title="
                      typeof action.title !== 'undefined' && action.title
                        ? action.title
                        : ''
                    "
                  >
      ...
      if (this.massActionType === "delete") {
        for (let i in this.massActionTargets) {
          if (this.massActionTargets[i].type === "delete") {
            ...
            this.massActionConfirmText = this.massActionTargets[i].confirm_text
        ...
        if (this.filters.length > 0) {
          ...
          if (column === "sort") {
            ...
            if (sort_exists === false) {
              if (this.currentSort === null) this.currentSort = this.sortAsc;
        ...
        switch (obj.column) {
          ...
          case "channel":
            ...
            if ("channels" in this.extraFilters) {
              obj.prettyValue = this.extraFilters["channels"].find(
                (channel) => channel.code == obj.val
              );
        ...
        if (this.gridCurrentData.hasOwnProperty("data")) {
          for (let currentData in this.gridCurrentData.data) {
            ...
            for (let currentId in this.gridCurrentData.data[currentData]) {
              if (i == 0) {
                this.dataIds.push(
                  this.gridCurrentData.data[currentData][this.filterIndex]
                );
              }
