...
@switch($type)
    ...
    @case('select')
        ...
        >
            <select
                ...
                v-bind="field"
                :class="[errors.length ? 'border !border-red-500' : '']"
                {{ $attributes->except(['value', ':value', 'v-model', 'rules', ':rules', 'label', ':label'])->merge(['class' => 'custom-select mb-1.5 w-full rounded-lg border border-[#E9E9E9] bg-white px-5 py-3 text-sm text-gray-600 shadow transition-all hover:border-gray-400 focus-visible:outline-none']) }}
            >
                {{ $slot }}
            </select>
        </v-field>
    ...
    @case('multiselect')
        <v-field
            ...
            {{ $attributes->except([])->merge(['class' => 'rounded-lg-md flex min-h-20 w-full flex-col border border-[#E9E9E9] bg-white px-5 py-3 text-sm font-normal text-gray-600 transition-all hover:border-gray-400']) }}
            name="{{ $name }}"
            multiple
        >
            {{ $slot }}
