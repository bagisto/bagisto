        ...
        @foreach ($menu->items as $menuItem)
            @foreach ($menuItem['children'] as $subMenuItem)
                <a href="{{ $subMenuItem['url'] }}">
                    <div
                        class="flex px-[25px] py-[20px] justify-between border-t-[1px] border-[#E9E9E9] hover:bg-[#f3f4f682] cursor-pointer items-center {{ request()->routeIs($subMenuItem['route']) ? 'bg-[#E9E9E9] rounded-t-[6px]' : '' }}"
                    >
