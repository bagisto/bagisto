...
<footer
    ...
    @endif
>
    <div class="flex justify-between gap-x-6 gap-y-8 p-15 max-1060:flex-col-reverse max-md:gap-5 max-md:p-8 max-sm:px-4 max-sm:py-5">
        <!-- For Desktop View -->
        <div
        ...
        >
            @foreach ($columns as $footerLinkSection)
                <ul class="grid gap-5 wrap-break-word text-sm">
        ...
        >
            ...
            <x-slot:content class="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] items-start gap-x-6 gap-y-8 bg-transparent! p-4!">
                @foreach ($columns as $footerLinkSection)
                    <ul
                        class="grid gap-5 wrap-break-word text-sm"
                        v-pre
                    >
                        @foreach ($footerLinkSection as $link)
                            <li>
        ...
        @if (core()->getConfigData('customer.settings.newsletter.subscription'))
            <div class="grid content-start gap-2.5">
                <p
                    class="max-w-72 text-3xl italic leading-11.25 text-navyBlue max-md:text-2xl max-sm:text-lg"
                    role="heading"
                    aria-level="2"
                >
                ...
                <div>
                    <x-shop::form
                        ...
                        class="mt-2.5 rounded-sm max-sm:mt-0"
                    ...
                    >
                        <div class="relative w-full">
                            <x-shop::form.control-group.control
                                type="email"
                                class="block w-105 max-w-full rounded-xl border-2 border-[#e9decc] bg-[#F1EADF] px-5 py-4 text-base max-1060:w-full max-md:p-3.5 max-sm:mb-0 max-sm:rounded-lg max-sm:border-2 max-sm:p-2 max-sm:text-sm"
                                name="email"
    ...
    <div class="flex justify-between bg-[#F1EADF] px-15 py-3.5 max-md:justify-center max-sm:px-5">
