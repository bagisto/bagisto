...
<footer class="mt-9 bg-lightOrange max-sm:mt-10 bg-secondary">
    <div class="flex justify-between gap-x-6 gap-y-8 p-[60px] max-1060:flex-col-reverse max-md:gap-5 max-md:p-8 max-sm:px-4 max-sm:py-5">
        ...
        <x-shop::accordion
            :is-active="false"
            class="hidden !w-full rounded-xl !border-2 max-1060:block max-sm:rounded-lg border-color-primary"
        >
            <x-slot:header class="rounded-t-lg bg-[#F1EADF] font-medium max-md:p-2.5 max-sm:px-3 max-sm:py-2 max-sm:text-sm bg-primary">
                        ...
                        <ul class="grid gap-5 text-sm">
                            ...
                            @foreach ($footerLinkSection as $link)
                                <li>
                                    <a
                                        href="{{ $link['url'] }}"
                                        class="text-sm font-medium max-sm:text-xs text-white">
                                        {{ $link['title'] }}
                                    </a>
                                </li>
                            @endforeach
        ...
        @if (core()->getConfigData('customer.settings.newsletter.subscription'))
            <div class="grid gap-2.5">
                <p
                    class="max-w-md text-3xl italic leading-[45px] text-white max-md:text-2xl max-sm:text-lg"
                    role="heading"
                    aria-level="2"
                >
                    ...
                    >
                        <div class="relative w-full">
                            <x-shop::form.control-group.control
                                ...
                                placeholder="subscription@mobista.in"
                            />
    ...
    <div class="flex justify-between px-[60px] py-3.5 max-md:justify-center max-sm:px-5 bg-primary">
        {!! view_render_event('bagisto.shop.layout.footer.footer_text.before') !!}

        <p class="text-sm text-zinc-950 max-md:text-center">
