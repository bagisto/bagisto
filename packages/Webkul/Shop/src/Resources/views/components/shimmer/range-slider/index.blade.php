<div>
    <div class="flex gap-[15px] items-center">
        <p class="shimmer w-12 h-[21px]"></p>

        <p class="shimmer w-[112px] h-[21px]"></p>
    </div>

    <!-- Price range slider effect -->
    <div class="flex justify-center relative items-center w-full h-20 mx-auto p-2">

        <div class="shimmer relative w-full h-1 bg-gray-200 rounded-2xl">
            <div class="shimmer absolute -top-[10px] -left-[4px] w-6 h-6 rounded-full"></div>
            <div class="shimmer absolute -top-[10px] -right-[4px] w-6 h-6 rounded-full"></div>
        </div>

    </div>
</div>
