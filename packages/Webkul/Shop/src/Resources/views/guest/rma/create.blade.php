...
<x-shop::layouts>
    <!-- Title of the page -->
    <x-slot:title>
        @lang('shop::app.rma.guest.create.title')
    </x-slot>
        ...
        <div class="container mt-8 px-16 max-lg:px-4">
            ...
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-2xl max-lg:text-base font-medium">
                    @lang('shop::app.rma.guest.create.heading')
                </h2>
        ...
        <script type="text/x-template" id="rma-request-template">
            <div class="md:border-2 bg-white box-shadow rounded-md mb-2 p-4">
                ...
                <div class="max-md:hidden">
                    <x-shop::datagrid :src="route('shop.guest.account.rma.create')" >
                        <!-- Datagrid Header -->
                        <template #header="{
                            isLoading,
                            available,
            ...
            app.component('rma-request-wrapper', {
                ...
                methods: {
                    ...
                    async rmaSubmit(params, { resetForm, setErrors }) {
                        ...
                        try {
                            const response = await this.$axios.post("{{ route('shop.guest.account.rma.store') }}", formData);

                            this.$emitter.emit('add-flash', { type: 'success', message: response.data.messages });

                            setTimeout(() => {
                                window.location.assign("{{ route('shop.guest.account.rma.index') }}");
                            }, 2000);
                        } catch (error) {
            ...
            app.component('v-order-items-list', {
                ...
                methods: {
                    ...
                    getOrderItems(orderId) {
                        let url = '{{route("shop.rma.action.ordered.product", ":orderId")}}';
