                    ...
                    @if (is_array($element)) 
                        @foreach ($element as $page => $url) 
                            ...
                            <li>
                                <a 
                                    href="{{ $isCurrent ? '#' : $url }}"
                                    class="flex h-9.25 w-8.75 items-center justify-center border border-zinc-200 text-center font-medium leading-normal text-black hover:bg-gray-100 {{ $isCurrent ? 'bg-zinc-200 pointer-events-none' : '' }}"
                                    aria-label="{{ $page }}"
