<div class="flex gap-[65px] p-[25px] items-center border-b-[1px] border-[#E9E9E9]">
    <div class="flex gap-x-[20px]">
        ...
        <div class="grid gap-y-[10px]">
            ...
            <p class="text-[16px]">
                @lang('shop::app.customer.account.wishlist.color') 
                {{ $item->product->color }}
            </p>

            <x-shop::form
                id="wishlist-{{ $item->id }}" 
                action="{{ route('shop.customer.wishlist.remove', $item->id) }}" 
            >
                @method('DELETE')
            ...
            @auth('customer')
                ...
                >
                    @lang('shop::app.customer.account.wishlist.remove')
                </a>
            @endauth
        </div>
    </div>
    ...
    <a href="{{ route('shop.customer.wishlist.move', $item->id) }}"> 
        ...
        <div
            class="m-0 ml-[0px] block mx-auto bg-navyBlue text-white text-base w-max font-medium py-[11px] px-[43px] rounded-[54px] text-center">
                @lang('shop::app.customer.account.wishlist.move-to-cart')
        </div>
    </a>
</div>
