                    ...
                    <tbody>
                        @foreach ($downloadableLinkPurchased as $item)
                            <tr class="bg-white border-b ">
                                <th scope="row" class="px-6 py-[16px] font-medium whitespace-nowrap text-black"> {{ $item->order_id }} </th>
                                <td class="px-6 py-[16px] text-black font-medium "> {{ $item->product_name }} </td>
                                <td class="px-6 py-[16px] text-black font-medium "> {{ $item->created_at }} </td>
                                <td class="px-6 py-[16px] text-black font-medium "> 
                                    @switch( $item->status )
                                        @case('completed')
                                            <span class=" text-white text-[12px] px-[10px] py-[4px] rounded-[12px] bg-[#5BA34B]"> {{ $item->status }} </span>
                                            @break
                                        @case('pending')
                                            <span class=" text-white text-[12px] px-[10px] py-[4px] rounded-[12px] bg-[#FDB60C]"> {{ $item->status }} </span>
                                            @break
                                    @endswitch
                                </td>
                                <td class="px-6 py-[16px] text-black font-medium "> 
