<div v-if="is_customer_exist">
    ...
    <div class="control-group" id="login-and-forgot-btn">
        <div class="forgot-password-link"  style="float: right; margin-right: 503px; margin-top: 11px;">
            <a href="{{ route('shop.customers.forgot_password.create') }}">{{ __('shop::app.customer.login-form.forgot_pass') }}</a>
            ...
            <div class="mt-10">
                @if (
                    ...
                    <a href="{{ route('shop.customers.resend.verification_email', Cookie::get('email-for-resend')) }}">{{ __('shop::app.customer.login-form.resend-verification') }}</a>
                @endif
            </div>
        </div>
