...
async function updateProductHeight(adminPage: Page, value: string) {
    const productEditPage = new ProductEditPage(adminPage);

    await productEditPage.openProduct(product.name);

    await productEditPage.fillInput("height", value);
...
async function createRuleAndVerify({
    ...
    await updateProductHeight(adminPage, productValue);
