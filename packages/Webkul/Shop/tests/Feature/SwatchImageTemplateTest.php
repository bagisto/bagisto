...
beforeEach(function () {
    ...
    Storage::put($this->path, (string) image_manager()->create(300, 300)->fill('33aa66')->toPng());
...
it('should link an image swatch to its stored file on a disk that is not local', function () {
    ...
    expect($option['swatch_value'])->toBe('https://cdn.example.com/'.$this->path)
        ->and($option['swatch_image']['large_image_url'])->toBe('https://cdn.example.com/'.$this->path);
});
