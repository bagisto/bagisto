...
class Bundle extends AbstractType
{
    ...
    public function prepareForCart($data)
    {
        ...
        foreach ($this->getCartChildProducts($data) as $productId => $data) {
            ...
            $products[0]['price'] += $cartProduct[0]['price'] * $data['quantity'];
            $products[0]['price_incl_tax'] += $cartProduct[0]['price_incl_tax'] * $data['quantity'];
            $products[0]['base_price'] += $cartProduct[0]['base_price'] * $data['quantity'];
            $products[0]['base_price_incl_tax'] += $cartProduct[0]['base_price_incl_tax'] * $data['quantity'];
            $products[0]['total'] += $cartProduct[0]['total'];
            $products[0]['total_incl_tax'] += $cartProduct[0]['total_incl_tax'];
            $products[0]['base_total'] += $cartProduct[0]['base_total'];
            $products[0]['base_total_incl_tax'] += $cartProduct[0]['base_total_incl_tax'];
