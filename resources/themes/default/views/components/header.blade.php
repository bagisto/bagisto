            ...
            <div class="flex items-center gap-4 md:gap-8 mt-2 md:mt-0 flex-wrap">
                ...
                <div class="relative inline-block text-left">
                    <button id="localeBtn" class="flex items-center gap-2 px-4 py-2 border rounded hover:bg-gray-100">
                        <img src="https://flagcdn.com/w20/{{ $flags[$currentLocale] ?? 'us' }}.png"
                    ...
                    <div id="localeDropdown" class="absolute mt-2 w-36 bg-white border rounded shadow-lg hidden z-50">
                        ...
                        <a href="{{ route('switch.language',$locale->code) }}">
                            <div class="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center gap-2">
                                <img src="https://flagcdn.com/w20/{{ $flags[$locale->code] ?? 'us' }}.png"
                                    class="w-5 h-5 rounded-full">
                                {{ $locale->name }}
                            </div>
                        </a>
    ...
    <div class="large-container mx-auto">
        <div class="flex items-center justify-between py-4 md:py-5 flex-wrap md:flex-nowrap">
            ...
            <div class="flex items-center gap-4 md:gap-8 w-full md:w-auto justify-between">
                ...
                {{-- @if(Route::currentRouteName() == 'shop.home.index' || Route::currentRouteName() ==
                'sbt.perfume.index' || Route::currentRouteName() == 'sbt.perfumes.search' || Route::currentRouteName()
                == 'spa.product.index' || Route::currentRouteName() == 'spa.product.search'
                || Route::currentRouteName() == 'flower.product.index' || Route::currentRouteName() ==
                'flower.product.search')
            ...
            <div class="flex items-center gap-4 lg:gap-10 mt-3 lg:mt-0">
                ...
                <nav class="hidden lg:flex items-center gap-10 text-[#371E0F]">
                    <a href="{{ route('shop.home.index') }}"
                        class="nav-link {{ Route::currentRouteName()=='shop.home.index' ? 'active' : '' }}">{{
                        __('home.nav.home') }}</a>
                    <a href="{{ route('shop.home.aboutus') }}"
                        class="nav-link {{ Route::currentRouteName()=='shop.home.aboutus' ? 'active' : '' }}">{{
                        __('home.nav.about') }}</a>
                    <a href="{{ route('shop.home.all.services') }}"
                        class="nav-link {{ Route::currentRouteName()=='shop.home.all.services' ? 'active' : '' }}">{{
                        __('home.nav.service') }}</a>
                    <a href="{{ route('shop.gallery.index') }}"
                        class="nav-link {{ Route::currentRouteName()=='shop.gallery.index' ? 'active' : '' }}">{{
                        __('home.nav.gallery') }}</a>
                    <a href="{{ route('shop.home.contactus') }}"
                        class="nav-link {{ Route::currentRouteName()=='shop.home.contactus' ? 'active' : '' }}">{{
                        __('home.nav.contact') }}</a>
                </nav>

                {{-- HAMBURGER MENU FOR MOBILE & TABLET --}}
                <div class="flex lg:hidden items-center cursor-pointer" id="mobile-menu-toggle">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="#371E0F" class="w-6 h-6">
                ...
                <div class="relative cursor-pointer">
                    ...
                    @auth('customer')
                    <!-- Account Icon -->
                    <img src="{{ asset('images/account_open.png') }}" alt="Account" class="w-6 h-6"
