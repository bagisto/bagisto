...
<section class="services-treatment-sec-wrap py-16 md:py-28 B">
    <div class="container">
        <h2
            class="font-oswald font-normal text-[36px] md:text-[60px] leading-[100%] uppercase text-center text-[#371E0F] mb-10 md:mb-14">
            Services Treatment
        </h2>
        ...
        <div class="flex items-center justify-center gap-2 mb-10 md:mb-14">
            ...
            <div id="categoryTabs"
                class="flex gap-4 md:gap-6 overflow-x-auto scroll-smooth no-scrollbar max-w-full md:max-w-[800px]">
                @foreach($categories as $category)
                @php $slug = $category->translations->first()->slug ?? ''; @endphp
                <a href="javascript:void(0);" data-slug="{{ $slug }}" class="px-2 py-2 whitespace-nowrap text-center font-roboto text-[16px] leading-[24px] tracking-[0.02em] transition-colors
        ...
        <div id="servicesContainer" class="services-treatment-grid-wrap grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            @forelse($services as $service)
            <div
                class="services-treatment-grid-col flex flex-col sm:flex-row bg-white rounded-2xl overflow-hidden border border-[#CACACA]">
                <div class="services-treatment-img-wrap relative w-full sm:w-[180px] h-[220px] sm:h-auto flex-shrink-0">
                    ...
                    @if($service->duration)
                    <span
                        class="absolute top-3 left-3 bg-white text-[#371E0F] font-roboto text-[14px] px-3 py-1 rounded-full">
                        {{ $service->duration }}
                    </span>
                    @endif
                </div>
                <div class="services-treatment-grid-body flex flex-col flex-1 p-4 md:p-6">
                    <div class="services-treatment-header flex justify-between items-start mb-2">
                        <div class="services-treatment-card-heading-wrap">
                            <span
                                class="services-treatment-heading-wrap font-oswald font-medium text-[24px] leading-[100%] tracking-[0.1em] uppercase text-[#371E0F] block">
                                {{ $service->name }}
                            </span>
                            <span class="services-treatment-slug-wrap font-roboto text-[14px] text-[#C9C0D4]">{{
                                $service->slug }}</span>
                        </div>
                        <span class="services-treatment-price-wrap font-oswald text-[20px] text-[#DFAA8B] uppercase">
                    ...
                    <a href="{{ route('shop.home.service.details',$service->product->url_key) }}">
                        <button
                            class="services-treatment-btn bg-[#D7CDBA] text-[#371E0F] font-roboto font-medium text-[14px] rounded-full w-fit">
                            Book Now
                        </button>
                    </a>
                </div>
