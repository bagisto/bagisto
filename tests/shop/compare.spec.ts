...
test('Add', async ({page}) => {
    ...
    await expect(page.getByText('Item added successfully to compare list').first()).toBeVisible();
});
...
test('Remove all', async ({page}) => {
    ...
    await expect(page.getByText('All items removed successfully.').first()).toBeVisible();
});
