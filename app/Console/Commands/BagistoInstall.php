...
class BagistoInstall extends Command
{
    ...
    public function handle()
    {
        ...
        $APP_NAME = $this->ask('Please enter name of application instance?');
        $this->comment($APP_NAME ?? 'Bagisto');
        $this->changeEnvironmentVariable('APP_NAME', $APP_NAME);

        $APP_ENV = $this->choice('Choose environment for this instance of Bagisto?', ['development', 'production'], 0);
        ...
        $APP_URL = $this->ask('Please enter application URL (Optional, default = localhost)?');
        $this->comment($APP_URL ?? 'localhost');
        $this->changeEnvironmentVariable('APP_URL', $APP_URL);

        $DB_CONNECTION = $this->ask('Enter database connection (Optional, default = mysql)?');
        $this->comment($DB_CONNECTION ?? 'mysql');
        $this->changeEnvironmentVariable('DB_CONNECTION', $DB_CONNECTION);

        $DB_HOST = $this->ask('Enter database host (Optional, default = 127.0.0.1)?');
        $this->comment($DB_HOST ?? '127.0.0.1');
        $this->changeEnvironmentVariable('DB_HOST', $DB_HOST);

        $DB_PORT = $this->ask('Enter database port (Optional, default = 3306)?');
        ...
        while (!isset($DB_DATABASE)) {
            $DB_DATABASE = $this->ask('Enter name of database?');
        ...
        $this->changeEnvironmentVariable('DB_USERNAME', $DB_USERNAME);
